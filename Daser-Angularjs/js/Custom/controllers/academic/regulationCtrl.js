...
app.controller("regulationCtrl",function($scope,$http,$uibModal, $log,$filter,$document,$timeout,Data,$ocLazyLoad,reg_Data)
              {
  ...
  $scope.x=$scope.contacts;
    ...
    var date = new Date();
           $scope.regulations.dates = $filter('date')(new Date(), 'yyyy-MM-dd');
            $scope.regulations.timestamp =  $filter('date')(new Date(), 'hh:mm:ss a');
  ...
  $ctrl.open = function (size, parentSelector,id,index) {
    ...
    var modalInstance = $uibModal.open({
      ...
      templateUrl: 'views/Academics_1/popups/regulation.html',
      controller: 'regulation_ModalInstanceCtrl',
      controllerAs: '$ctrl',
      size: size,
    ...
    $scope.addData=function(){  
       $scope.loading=true; 
        ...
        if($scope.regulations.regulation=="" || $scope.regulations.course=="" )
        { 
           counter++  
        }
       ...
       $scope.item=$http({
            method: 'POST',
            url: regulation_URL,
            data:{a:$scope.regulations,b:$scope.x},
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
                        }).then(
          function successCallback(response) 
           {
              ...
              $scope.loading=false; 
               if(response.data.errors)
                ...
                $timeout(function () {  $scope.fail=true;}, 3000);  
                    ...
                    {
                              $scope.regulations.regulation="";
                                $scope.regulations.course="";
...
app.controller('regulation_ModalInstanceCtrl', function ($uibModalInstance,$http, items,reg_singleData,$timeout,Data) {
  var $ctrl = this;
  $ctrl.s = items;
  $ctrl.selected = {
    item: $ctrl.s
    ...
    $ctrl.update={code:'',course:''};
    
     $ctrl.thing="sddsd";
            reg_singleData.single_global_getdata($ctrl.s).then(function(data)
        {
            $ctrl.thing=data;
           
    $ctrl.update.code=$ctrl.thing[0].regulation; 
        });
    Data.global_getdata().then(function(data)
        {
           $ctrl.thing=data;
...
app.service('reg_Data', function($http) {  
    return {reg_global_getdata:function(){
   return $http({
  method: 'GET',
  url: regulation_URL,
}).then(function successCallback(response) {return response.data.regulation;}, function errorCallback(response) {  return response.data.regualtion;}); 
    }
 }  
});

app.service('reg_singleData', function($http) {  
    return {single_global_getdata:function(id){
   return $http({
  method: 'GET',
  url: regulation_URL+"/"+id,
